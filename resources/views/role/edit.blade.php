<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Editar grupo de acesso: {{ $role->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @endif
            <form class="bg-white p-6 rounded w-full" action="{{ route('roles.update', ['role' => $role->id]) }}" method="post">
                @csrf
                @method('PUT')
                <div class="w-full md:w-1/2 mb-10">
                    ...
                    <input name="name" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 @error('name') border border-red-500 @enderror" id="grid-last-name" type="text" value="{{ old('name', $role->name) }}">
                    @error('name')
                                ...
                                <div class="flex">
                                    @foreach($group->permissions as $permission)
                                        <div class="flex items-center ml-4">
                                            <input id="permission_id-{{ $permission->id }}" value="{{ $permission->id }}" name="permission_id[]" type="checkbox" {{ $role->permissions->contains($permission->id) ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">

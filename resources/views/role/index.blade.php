<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Gerenciamento de grupos de acesso') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="header mb-5 flex justify-end">
                <a href="{{ route('roles.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Adicionar grupo de acesso</a>
            </div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        @forelse($roles as $role)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-right space-x-3">
                                    <a href="{{ route('roles.edit', ['role' => $role->id]) }}" class="font-medium text-blue-600 hover:underline">Editar</a>
                                    <a href="{{ route('users.delete', ['user' => $role->id]) }}" onclick="event.preventDefault(); document.getElementById('form-delete-{{ $role->id }}').submit()" class="font-medium text-red-600 hover:underline">Excluir</a>
                                    <form action="{{ route('roles.delete', ['role' => $role->id]) }}" method="post" id="form-delete-{{ $role->id }}" class="hidden">
                                        @csrf
                                        @method('delete')
                                    </form>
                                </td>
                        ...
                        @empty
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td>Nenhum grupo de acesso encontrado</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>

    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="header mb-5 flex justify-end">
                <a href="{{ route('services.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Adicionar ordem de serviço</a>
            </div>
                        ...
                        @forelse($services as $service)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-right space-x-3">
                                    @can('Editar ordens de serviços')
                                        <a href="{{ route('services.edit', ['service' => $service->id]) }}" class="font-medium text-blue-600 hover:underline">Editar</a>
                                    @endcan
                                    @can('Excluir ordens de serviços')
                                        <a href="{{ route('services.delete', ['service' => $service->id]) }}" onclick="event.preventDefault(); document.getElementById('form-delete-{{ $service->id }}').submit()" class="font-medium text-red-600 hover:underline">Excluir</a>
                                    @endcan
                                    <form action="{{ route('services.delete', ['service' => $service->id]) }}" method="post" id="form-delete-{{ $service->id }}" class="hidden">
                                        @csrf
                                        @method('delete')
                                    </form>
                                </td>
